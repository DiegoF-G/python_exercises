...
cabecalho('Sets não possuem elementos repetidos e caso tenha eles são "unificados"', 50)
...
cabecalho('Sets podem possuir dados de diferentes tipos e são mutáveis', 70)
onix = {2017, "Onix", 37123.04, True}
print(onix)
onix.add('1.0 turbo')
print(onix)

cabecalho('Sets não são enumeráveis (não são indexados), diferente do que pode ser um conjunto na Matemática', 120)
try:
    print(setCarros[0])
finally:
